 ...
 * 根式(rad)、定界符(d)、N 元运算符(nary)、重音符号(acc)、横线(bar)、组合字符(groupChr)、
...
public final class OmmlToLatexConverter {
    ...
    private static StringBuilder convertNode(Node node) {
        ...
        } else if ("groupChr".equals(localName)) {
            return convertGroupCharacter(el);
    ...
    private static StringBuilder convertSuperscript(Element el) {
        ...
        StringBuilder baseLatex = base != null ? convertChildren(base) : new StringBuilder();
        // \\overbrace{...}^{...} 会把上标当成居中的花括号注释。
        // OMML 的外层 sSup 表示普通右上角上标，因此先把 groupChr 整体分组。
        boolean groupCharacterBase = base != null && findChild(base, "groupChr") != null;
        return new StringBuilder()
                .append(groupCharacterBase ? "{" : "")
                .append(baseLatex)
                .append(groupCharacterBase ? "}" : "")
                .append("^{")
    ...
    /**
     * 转换组合字符元素，例如位于表达式上方或下方的可伸缩花括号。
     *
     * <p>Word 使用 {@code m:groupChr} 表示截图中的“在 b×d 上方加大括号”。
     * 花括号使用 LaTeX 原生的 overbrace/underbrace，其他组合字符保留为
     * overset/underset，避免递归处理时只剩下正文而丢失符号。</p>
     */
    private static StringBuilder convertGroupCharacter(Element el) {
        Element properties = findChild(el, "groupChrPr");
        String position = "top";
        String character = "";
        if (properties != null) {
            Element pos = findChild(properties, "pos");
            if (pos != null && !getAttrVal(pos).isEmpty()) position = getAttrVal(pos);
            Element chr = findChild(properties, "chr");
            if (chr != null) character = getAttrVal(chr);
        }

        boolean below = "bot".equals(position);
        if (character.isEmpty()) character = below ? "⏟" : "⏞";
        Element expression = findChild(el, "e");
        String content = expression == null ? "" : convertChildren(expression).toString();

        if ("⏞".equals(character) || "⏟".equals(character)) {
            return new StringBuilder(below ? "\\underbrace{" : "\\overbrace{")
                    .append(content).append("}");
        }
        return new StringBuilder(below ? "\\underset{" : "\\overset{")
                .append(latexEscape(character)).append("}{").append(content).append("}");
    }
