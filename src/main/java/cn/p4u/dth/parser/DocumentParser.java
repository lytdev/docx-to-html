 ...
 * </ul>
 * <p>
 * 主要使用场景：作为三阶段管线的第二阶段，由 {@link cn.p4u.dth.converter.DocxConverter}
...
public final class DocumentParser {
    ...
    private DocumentParser(Path extractedDir) {
        this.packageRoot = extractedDir.toAbsolutePath().normalize();
    ...
    public static DocumentModel parse(Path extractedDir) {
        return parse(extractedDir, null);
    }
    ...
    public static DocumentModel parse(Path extractedDir, FileParseCallback<ContentBlock> callback) {
        ...
        if (!Files.exists(docFile)) {
            DocxConversionException ex =
                    new DocxConversionException("document.xml not found in extracted directory");
        ...
        try {
            ...
            int total = countParseableBodyItems(children);
            int count = 0;
            for (int i = 0; i < children.getLength(); i++) {
                ...
                if (W.equals(el.getNamespaceURI())) {
                    int line = count + 1;
                    try {
                        ContentBlock parsed = null;
                        if ("p".equals(localName)) {
                            parsed = parser.parseParagraph(el);
                        } else if ("tbl".equals(localName)) {
                            parsed = parser.parseTable(el);
                        }
                        if (parsed != null) {
                            contentBlocks.add(parsed);
                            count++;
                            if (callback != null) {
                                String type = parsed instanceof ParagraphBlock ? "paragraph" : "table";
                                callback.onLineParsed(count, total,
                                        new CallBackRecord<ContentBlock>(count, type, parsed));
                            }
                        }
                    } catch (RuntimeException ex) {
                        errorReported = true;
                        if (callback != null) callback.onError(ex, line);
                        throw ex;
                    }
                }
            }
            ...
            if (!errorReported && callback != null) callback.onError(e, 0);
            throw e;
        } catch (Exception e) {
            if (!errorReported && callback != null) callback.onError(e, 0);
    ...
    private ParagraphBlock parseParagraph(Element pEl) {
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            if (W.equals(ns) && "pPr".equals(localName)) {
                ...
                // 嵌入对象（旧版 OLE 对象内的图片）
                ParagraphElement object = parseObject(el);
    ...
    private void extractRunContent(Element rEl, List<ParagraphElement> elements, String paraStyleId) {
        ...
        for (int i = 0; i < runChildren.getLength(); i++) {
            ...
            if (W.equals(rns) && "drawing".equals(rln)) {
                ...
                flushTextParts(textParts, rEl, elements, paraStyleId);
                ParagraphElement object = parseObject(rce);
                if (object != null) elements.add(object);
    ...
    private boolean extractContentFromContainer(Element container, List<ParagraphElement> elements) {
        ...
        for (int i = 0; i < objectNodes.getLength(); i++) {
            ...
            ParagraphElement object = parseObject(objEl);
            if (object != null) {
                elements.add(object);
                found = true;
            }
     ...
     * @return 普通 OLE 返回图片元素，MathType/公式编辑器对象返回公式元素；无预览图时返回 null
     */
    private ParagraphElement parseObject(Element objectEl) {
        ...
        if (shapeNodes.getLength() > 0) {
            ImageElement preview = parseVmlShape((Element) shapeNodes.item(0));
            if (preview != null && isFormulaOleObject(objectEl)) {
                // 可编辑数据可能已过时；与当前预览明显冲突时只保留图片。
                return new MathElement(extractOleLatex(objectEl, preview), null, preview.mediaPath(), preview.mimeType(),
    ...
    private static int countParseableBodyItems(NodeList children) {
        int total = 0;
        for (int i = 0; i < children.getLength(); i++) {
            Node child = children.item(i);
            if (child instanceof Element el && W.equals(el.getNamespaceURI())
                    && ("p".equals(el.getLocalName()) || "tbl".equals(el.getLocalName()))) {
                total++;
            }
        }
        return total;
    ...
    /**
     * 根据 OLE 程序标识判断嵌入对象是否来自公式编辑器。
     * 常见值包括 Equation.DSMT4（MathType）、Equation.3 和包含 MathType 的变体。
     */
    private boolean isFormulaOleObject(Element objectEl) {
        NodeList oleObjects = objectEl.getElementsByTagNameNS("*", "OLEObject");
        for (int i = 0; i < oleObjects.getLength(); i++) {
            String progId = attributeByLocalName((Element) oleObjects.item(i), "ProgID");
            if (progId == null) continue;
            String normalized = progId.toLowerCase(Locale.ROOT);
            if (normalized.contains("equation")
                    || normalized.contains("mathtype")
                    || normalized.contains("dsmt")
                    || normalized.contains("mathmagic")
                    || normalized.contains("axmath")
                    || normalized.contains("wiris")
    ...
    /** 只读取本 DOCX 包内的 OLE 文件，不跟随外部链接。提取失败不影响图片显示。 */
    private String extractOleLatex(Element objectEl, ImageElement preview) {
        NodeList objects = objectEl.getElementsByTagNameNS("urn:schemas-microsoft-com:office:office", "OLEObject");
        for (int i = 0; i < objects.getLength(); i++) {
            Element ole = (Element) objects.item(i);
            if ("Link".equalsIgnoreCase(ole.getAttribute("Type"))) continue;
            RelsParser.Rel relation = rels.get(ole.getAttributeNS(R, "id"));
            if (relation == null || !"oleObject".equals(relation.type())) continue;
            try {
                Path file = packageRoot.resolve("word").resolve(relation.target()).normalize();
                if (!file.startsWith(packageRoot) || !Files.isRegularFile(file)) continue;
                String latex = OleEquationLatex.extract(file);
                Path previewFile = packageRoot.resolve("word").resolve(preview.mediaPath()).normalize();
                if (previewFile.startsWith(packageRoot)
                        && FormulaPreviewConsistency.conflicts(previewFile, latex)) {
                    LOG.warning("OLE equation data conflicts with its preview; omitting data-latex: "
                            + relation.target());
                    return "";
                }
                return latex;
            } catch (RuntimeException ignored) {
                // 非法关系路径也不能阻断原有的预览图片处理。
            }
        }
        return "";
    }
