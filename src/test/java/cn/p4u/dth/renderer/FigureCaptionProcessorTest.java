...
class FigureCaptionProcessorTest {
    ...
    @Test
    void mathTypeOlePreviewIsRenderedAsFormulaImage() throws Exception {
        try (cn.p4u.dth.util.TestDocxBuilder builder = new cn.p4u.dth.util.TestDocxBuilder()) {
            builder.addContentTypes().addRels()
                    .addDocumentRels("<Relationship Id=\"rId1\" Type=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships/image\" Target=\"media/formula.png\"/>"
                            + "<Relationship Id=\"rId2\" Type=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships/image\" Target=\"media/object.png\"/>")
                    .addMedia("formula.png", new byte[]{1, 2, 3})
                    .addMedia("object.png", new byte[]{4, 5, 6})
                    .addDocument(oleObjectParagraph("rId1", "Equation.DSMT4")
                            + oleObjectParagraph("rId2", "Excel.Sheet.12"));
            try (var input = java.nio.file.Files.newInputStream(builder.build())) {
                Document result = Jsoup.parse(cn.p4u.dth.converter.DocxConverter.convert(input));
                ...
                var ordinary = result.selectFirst("img[data-type=image]");
                assertNotNull(ordinary);
                assertEquals("image-block image-item", ordinary.className());
            }
        }
    }

    private String oleObjectParagraph(String relationshipId, String progId) {
        return "<w:p><w:r><w:object>"
                + "<v:shape xmlns:v=\"urn:schemas-microsoft-com:vml\" style=\"width:20pt;height:10pt\">"
                + "<v:imagedata r:id=\"" + relationshipId + "\"/></v:shape>"
                + "<o:OLEObject xmlns:o=\"urn:schemas-microsoft-com:office:office\" "
                + "Type=\"Embed\" ProgID=\"" + progId + "\"/>"
                + "</w:object></w:r></w:p>";
    }
